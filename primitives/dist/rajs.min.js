...
return this.params[name];},set:function(key,value){key=encodeURI(key);value=encodeURI(value);var kvp=document.location.search.substr(1).split('&');var i=kvp.length;var x;while(i--)
{x=kvp[i].split('=');if(x[0]==key)
{x[1]=value;kvp[i]=x.join('=');break;}}
if(i<0){kvp[kvp.length]=[key,value].join('=');}
document.location.search=kvp.join('&');}};Params.init();var Events={events:{},callbacks:{},on:function(eventName,callback){if(!this.events[eventName])
this.events[eventName]=[];this.events[eventName].push(callback);},once:function(eventName,callback){if(!this.callbacks[eventName])
this.callbacks[eventName]=[];this.callbacks[eventName].push(callback);},off:function(eventName){if(this.events[eventName])
this.events[eventName]=[];if(this.callbacks[eventName])
this.callbacks[eventName]=[];},fire:function(eventName,args,context){if(!this.events[eventName])
this.events[eventName]=[];for(var event of this.events[eventName])
event.apply(context,args);if(!this.callbacks[eventName])
this.callbacks[eventName]=[];for(var event of this.callbacks[eventName])
event.apply(context,args);this.callbacks[eventName]=[];}};function estimation(num){if(num>999)
...
return hours+':'+minutes+':'+seconds;}function time(){return Math.floor((new Date()).getTime()/1000);}
